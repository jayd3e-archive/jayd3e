		...
		if ( !bubbling ) {
			...
			if ( !elem ) {
				...
				if ( jQuery.event.global[ type ] ) {
					// XXX This code smells terrible. event.js should not be
                    // directly
					// inspecting the data cache
					jQuery.each( jQuery.cache, function() {
						// internalKey variable is just used to make it easier
                        // to find
						// and potentially change this stuff later; currently it
                        // just
...
jQuery.extend({
	...
	ajaxSettings: {
         ...
         * null, cache: null, traditional: false, headers: {}, crossDomain:
         * null,
         */
		...
		if ( !s.crossDomain || jQuery.support.cors ) {
			...
			return {
				send: function( headers, complete ) {
					...
					// #5280: we need to abort on unload or IE will keep
                    // connections alive
						...
						if ( callback && ( isAbort || xhr.readyState === 4 ) ) {
							...
							if ( isAbort ) {
								...
								status =
									...
									// Webkit returns 0 for failing cross-domain
                                    // no matter the real status
									status === 0 ?
										(
											// Webkit, Firefox: filter out
                                            // faulty cross-domain requests
											!s.crossDomain || statusText ?
											(
												// Opera: filter out real aborts
                                                // #6060
												responseHeaders ?
												304 :
												0
											) :
											// We assume 302 but could be
                                            // anything cross-domain related
											302
										) :
										(
											// IE sometimes returns 1223 when it
                                            // should be 204 (see #1450)
											status == 1223 ?
												204 :
												status
										);
